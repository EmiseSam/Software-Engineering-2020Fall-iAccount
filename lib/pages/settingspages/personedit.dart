...
class _PersonEditState extends State<PersonEdit> {
  ...
  void initData() async {
    Member a = new Member("'自己'");
    var d = await dbHelper.insertMember(a);
    Member b = new Member("'父母'");
    var g = await dbHelper.insertMember(b);
    Member c = new Member("'兄弟姐妹'");
    await dbHelper.insertMember(c);
    Member f = new Member("'亲戚'", id: d);
    await dbHelper.insertMember(f);
    var e = await dbHelper.deleteMember(g);
    _personData = await dbHelper.getMember();
    setState(() {
      print('test');
      print(e);
      print(_personData[1].member);
    });
  }
